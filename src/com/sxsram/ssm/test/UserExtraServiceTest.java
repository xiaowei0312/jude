...
public class UserExtraServiceTest {
	...
	public void testFindUserList() throws Exception {
		List<UserExpand> userList = null;
		UserExpandQueryVo userExpandQueryVo = new UserExpandQueryVo();
		userList = userService.getUserList(userExpandQueryVo);
		//System.out.println(userList.size());
		
		for(UserExpand user:userList){
			UserExtra userExtra = new UserExtra();
	...
	@Test
	public void testFindUserInviterNum() throws Exception {
		...
		//items.add(new QueryConditionItem("u.phone", "13546712419", QueryConditionOp.EQ));
		userExtraQueryVo.setQueryCondition(new QueryCondition(items));
		userExtras = userExtraService.findUserExtrasTotalNumHasInviter(userExtraQueryVo);
		
		System.out.println(userExtras.size());
		for(UserExtra userExtra:userExtras){
			System.out.println(userExtra.getInviterNum() + "," + userExtra.getInviterUser().getPhone());
		}
	}
}
