...
var commodityListComp = {
	...
	editCommodityClicked : function(id, name) {
		$('#modal_edit #id').val(id);
		$('#modal_edit #name').val(name);
	},
	delCommodityClicked : function(commodityId) {
		if (confirm("确定删除该权限吗？") == false)
			return;
		$.ajax({
			type : "POST",
			url : commodityListComp.basePath
					+ "/perm/deletePermAjax.action",
			data : {
				id : commodityId,
			},
			success : function(e) {
